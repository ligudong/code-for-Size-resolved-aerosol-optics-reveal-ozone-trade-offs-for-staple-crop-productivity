...
# Aggregate provincial geometries into regional polygons
region_plot <- province_plot %>%
  st_make_valid() %>%
  group_by(region) %>%
  summarise(geometry = st_union(geometry))

# Read and merge city-level shapefiles
city <- province_plot %>%
  st_drop_geometry() %>%
...
# Combine nested shapefiles into single sf object
city <- bind_rows(
...
# Load gridded panel data and calculate variables
data <- qread("data/derived/tidied.qs", nthreads = qn) %>%
  mutate(
    ...
    O3 = fcase(
      crop == "Maize", O3_6,
      crop == "Rice", O3_7,
      crop == "Wheat", (O3_7 * (MA - `GR&EM` + 1) - O3_1) / (MA - `GR&EM` + 0)
    )
  )

# Convert to raster and compute annual mean by crop
yearmean <- data %>%
  lazy_dt() %>%
  group_by(x, y, crop) %>%
  summarise(across(c(contains("GOSIF"), Faod, Caod, O3, AOT40, fraction), fmean), .groups = "drop") %>%
  pivot_wider(names_from = crop, values_from = c(contains("GOSIF"), Faod, Caod, O3, AOT40, fraction)) %>%
  as_tibble() %>%
  rast(type = "xyz", crs = "epsg:4326")

# Loop through each crop and generate 6-panel composite maps
c("Maize", "Wheat", "Rice") %>%
  walk(function(acrop) {
    ...
    patch <- pmap(
      list(
        c("GOSIF_sum", "fraction", "Faod", "Caod", "O3", "AOT40"),
        list(
    ...
    ggsave(str_c("figures/supplementary/figureS1_3_background_", acrop, ".tif"), patch, width = 2, height = 3, scale = 7, device = "tiff", dpi = 600)
  })